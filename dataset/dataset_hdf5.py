...
class TrajectoryDatasetHDF5(BaseDataset):
    def __init__(self, config, dtype=torch.float32):
        ...
        self.extra_shapes = (
            config.get("extra_shapes")
            if "extra_shapes" in config.keys()
            else config.get("eval_shapes")
        )
        self.dict_norm_states = DictNormalizer(self.state_shapes)
        self.dict_norm_goals = DictNormalizer(self.goal_shapes)
        self.dict_norm_extras = DictNormalizer(self.extra_shapes)
    ...
    def __getitem__(self, idx):
        ...
        extra_shapes = self.extra_shapes

        for key in state_keys:
            data = hdf5_data[key][str(idx)][...]
            sample_state[key] = torch.tensor(data, dtype=dtype)
        ...
        if extra_shapes is not None:
            for key in extra_shapes:
                # Assume data stored as [num_samples, ...] shape in HDF5
                data = hdf5_data[key][str(idx)][...]
                sample_extra[key] = torch.tensor(data, dtype=dtype)
            return sample_state, sample_goal, sample_extra
    ...
    def get_stats_from_file(self):
        ...
        self.dict_norm_extras.set_stats_from_json(json_stats_dict_r)
        return self.dict_norm_states, self.dict_norm_goals, self.dict_norm_extras

    def get_normalizer_fit(self):
        hdf5_data = self.h5_file
        self.dict_norm_states.fit(hdf5_data)
        self.dict_norm_goals.fit(hdf5_data)
        return self.dict_norm_states, self.dict_norm_goals
...
class TrajectoryKDatasetHDF5(BaseDataset):
    def __init__(self, config, dtype=torch.float32):
        ...
        self.goal_shapes = config["goal_shapes"]
        self.expand_len = config["expand_goal"]
        self.extra_shapes = config.get("eval_shapes")
        self.dict_norm_states = DictNormalizer(self.state_shapes)
        self.dict_norm_goals = DictNormalizer(self.goal_shapes)
        self.dict_norm_extras = DictNormalizer(self.extra_shapes)
        ...
        for k in resampling_states:
            ...
            elif k in self.extra_shapes:
                d = int(self.extra_shapes[k]["shape"])
    ...
    def __getitem__(self, idx):
        ...
        # if "delta_pos" in resampling_states:
        #     resampling_states = resampling_states + list(self.extra_shapes.keys())
        sample_state = {}
        sample_goal = {}
        extra_shapes_dict = {}
        state_keys = self.state_shapes.keys()
...
class TrajectoryPadDatasetHDF5(TrajectoryDatasetHDF5):
    def __init__(self, config, dtype=torch.float32):
    ...
    def __getitem__(self, idx):
        ...
        t_np = hdf5_data["time"][str(idx)][...]
        t = np.squeeze(t_np, 1)
        orig_len_M = t.shape[0]
        start_idx = np.random.randint(0, (t.shape[0] - 2))
        mask = torch.zeros((sequence_len), dtype=torch.bool)  # , dtype=torch.bool)
        seq_len_L = orig_len_M - start_idx
        mask[:seq_len_L] = 1.0
        for key in state_keys:
            # randomly peak a start pos idx
            data = hdf5_data[key][str(idx)][start_idx:, :]
            sample_state[key] = self.padding_with_abs_tail(
